...
class DiskSpaceTest(TestClass):
    ...
    def run_test(self):
        ...
        if result < 1000:
            return TestStatus.Attention, u'There are {} MB of free space available. Please free up some space.'.format(result)
        elif result < 10:
            return TestStatus.Error, u'There are only {} MB of free space available. Please free up some space.'.format(result)
        else:
